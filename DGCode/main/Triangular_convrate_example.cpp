...
std::string filename = file("filename", "TriaL2convergence.txt");

std::cout<<"Reading parameters from "<<filename<<std::endl;

GetPot g2(filename.c_str());
std::size_t n_repetitions = g2("reps",0);
...
for (std::size_t j = 0; j < n_repetitions; j++) {
  ...
  Mesh Th(p1,p2,ne_s[j],ne_s[j],"Triangle");
...
std::vector<double> p;
std::cout << "Ne   Diameter   L2error" <<std::endl;
for (std::size_t k = 0; k < n_repetitions;k++)
  std::cout << ne_s[k] << "  " << diameters[k] << "  " << L2errors[k] << std::endl;
